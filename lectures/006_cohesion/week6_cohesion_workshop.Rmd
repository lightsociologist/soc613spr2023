...
``coreness(g)`` in **igraph** cacluates cores.
...
A clique is a maximally connected subgraph (e.g., every node is connected to one another) of at least three nodes 
...
plot.igraph(karate, vertex.color=V(karate)$leid_large_comms)
...
plot.igraph(karate, vertex.color=V(karate)$leid_small_comms)
